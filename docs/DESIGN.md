...
I'm not approaching this keyboard firmware with a statistical or scientific methodology — mostly informed guesses about what should go where, refined by daily use rather than rigorous testing. As a result, expect small inconsistencies or rough edges as the design continues to shift and settle. It might take me a while to develop some features and periodically run clean-up sprints (i.e., refactors) to improve the source code’s maintainability.
